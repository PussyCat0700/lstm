...
#SBATCH --output=./logs/ffnn.out
#SBATCH --error=./logs/ffnn.err
...
do
    # 替换 %d 为当前的数字 i
    ckpt_dir="/data1/yfliu/windpower_baseline/runs_china/ffnn/ffnn_$i"
    # 执行命令，传递参数并替换 %d
    python train.py 1 --plant_number $i --checkpoint_dir $ckpt_dir --num_epochs 1000
done
python gather_results.py 1