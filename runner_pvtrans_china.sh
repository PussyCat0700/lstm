...
#SBATCH --output=./logs/station_logs/china/pvtrans_e_%A_%a.out
#SBATCH --error=./logs/station_logs/china/pvtrans_e_%A_%a.err
...
do
    ...
    mkdir -p $logdir
    CUDA_VISIBLE_DEVICES=$gpu_id python train.py 5 --plant_set china --plant_number $plant_number --months $1 --batch_size 128 --period $2 > "$logdir/log.txt" 2>&1 &
done
wait