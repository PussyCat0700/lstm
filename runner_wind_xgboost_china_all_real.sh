...
#SBATCH --job-name=xgboost_china_all_real_wnd
...
#SBATCH --error=./logs/station_logs/china_all_real/xgboost_%A_%a.err
...
do
    ...
    logdir=/data0/yfliu/logs/solar/china_all_real/xgboost/${1}_${task_id}_${plant_number}
    echo "exporting to" "$logdir/log.txt"
    mkdir -p $logdir
    python sklearn_train.py 0 --plant_number $plant_number --months $1 --plant_set "china_all_real" --plant_type 0 --period $2 > "$logdir/log.txt" 2>&1 &
done
wait