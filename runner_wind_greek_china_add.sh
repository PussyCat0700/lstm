...
#SBATCH --job-name=greek_china_add_wnd
...
#SBATCH --error=./logs/station_logs/china_add/greek_%A_%a.err
...
do
    ...
    mkdir -p $logdir
    python sklearn_train.py 3 --plant_number $plant_number --months $1 --plant_set "china_add" --plant_type 0 > "$logdir/log.txt" 2>&1 &
done
wait