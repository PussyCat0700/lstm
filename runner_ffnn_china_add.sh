...
#SBATCH --output=./logs/station_logs/china_add/ffnn_%A_%a.out
#SBATCH --error=./logs/station_logs/china_add/ffnn_%A_%a.err
...
do
    ...
    mkdir -p $logdir
    CUDA_VISIBLE_DEVICES=$gpu_id python train.py 1 --plant_set china_add --plant_number $plant_number --months $1 --batch_size 1024 --period $2 > "$logdir/log.txt" 2>&1 &
done
wait