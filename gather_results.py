...
from paths import PLANTS, path_loader, read_station_info
...
parser.add_argument("--period", type=int, default=24)  # defaults to 24
...
for postfix in postfixes:
    ...
    for group_type, group_data in grouped:
        ...
        for idx, row in group_data.iterrows():
            station_serial = row['PLANT_NO']
            station_path, _ = path_loader.get_run_path_status(args.model)
            # Check if metrics.csv exists in the current subdirectory
            station_path_original = '/'.join(station_path.split('/')[:-1]+[f'{args.model}_{station_serial}',])
            station_path_stripped = '/'.join(station_path.split('/')[:-1]+[f'{args.model}_{station_serial.lstrip("0")}',])
            filtered = True
            # possible files
            for metrics_file in [os.path.join(x, f'metrics{postfix}.csv') for x in [station_path_original, station_path_stripped]]:
                if not filtered:
                    break
                if os.path.isfile(metrics_file):
                    # Read the metrics.csv and append to the list
                    df = pd.read_csv(metrics_file)
                    df['station'] = station_serial
                    filtered = False
                    if df["rmse"][0] >= 0:
            ...
            if filtered:
                all_filtered.append(station_serial)
            pbar.update()
